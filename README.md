# Overview
Agile Robotics for Industrial Automation Competition [ARIAC](http://gazebosim.org/ariac) is hosted by NIST (National Institute of Standard and Technology) and the objective of this competition is to improve the agility of industrial robot system, with the goal of enabling industrial robots on the shop floors to be more productive, more autonomous and be more responsive to the needs of shop floor workers. Gazebo provides the simulation infrastructure and the virtual environment. In order to test the agility of the task-level planner, despite picking and placing the desired part to a correct pose, it also may encounter various agility challenges developed based on input from industry representatives, like dropping the part, get higher priority order during the process, pick form conveyor belt etc.
