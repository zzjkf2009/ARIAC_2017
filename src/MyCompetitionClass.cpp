...
/**
 * [MyCompetitionClass::MyCompetitionClass Constructor of the MyCompetitionClass class, isfinite joint publisher ]
...
/**
 * [MyCompetitionClass::current_score_callback call back function for Sucscribing the score]
 * @param msg [Float32]
 */
...
/**
 * [MyCompetitionClass::competition_state_callback callback function for the competition state]
 * @param msg [string]
 */
...
/**
 * [MyCompetitionClass::order_callback callback function for the order message, read kit and part info, see order and kit struct in head file]
 * @param order_msg [Order]
 */
void MyCompetitionClass::order_callback(const osrf_gear::Order::ConstPtr & order_msg) {
        ...
        order Order;
        // if order_id (string) is different from last time, means new order is comming, so
        // read the kits and parts content of this order
        if(order_name.compare(order_msg->order_id) != 0) {
                osrf_gear::Order order_ = *order_msg;
                for(osrf_gear::Kit kit_ : order_.kits) {   // for each kit assign priority
                        Kit kit;
                        for(osrf_gear::KitObject object_ : kit_.objects) {  // for each parts
                                Order.pose = object_.pose;
                                Order.type = object_.type;
                                kit.order_in_kit_list.push_back(Order);
                                kit.priority = priority_num;
                        }
                        kit_priority_queue.insert(std::pair<int,Kit>(kit.priority,kit)); // pair the priority with the kit
                        priority_num++;
                }
...
/**
 * [MyCompetitionClass::orderPoseBroadcaster boardcaste the tf frame based on which agv is assigned to the kit]
 * @param msg      [Pose]
 * @param x        [int]
 * @param tray_num [int]
 */
...
/**
 * [MyCompetitionClass::desired_part_pose obtain the part world pose from the relative pose]
...
/**
 * [MyCompetitionClass::joint_state_callback joint callback]
 * @param joint_state_msg [JointState]
 */
...
/**
 * [MyCompetitionClass::logical_camera_callback callback function for logical camera]
 * @param image_msg [LogicalCameraImage]
 */
...
/**
 * [MyCompetitionClass::break_beam_callback callback function for beam sensor]
 * @param msg [Proximity]
 */
...
/**
 * [MyCompetitionClass::proximity_sensor_callback callback function for the proximity_sensor_callback]
 * @param msg [sensor_msgs]
 */
...
/**
 * [MyCompetitionClass::laser_profiler_callback callback function for the laser ]
 * @param msg [LaserScan]
 */
...
/**
 * [MyCompetitionClass::agv1_state_callback callback function to verfiy the state of the agv 1]
 * @param msg [String]
 */
...
/**
 * [MyCompetitionClass::agv1_state_callback callback function to verfiy the state of the agv 2]
 * @param msg [String]
 */
...
/**
 * [agv_faulty_part_detect callback function part_detect filters information about parts,from other transfromation in /tf]
 * @param msg [TFMessage]
 */
